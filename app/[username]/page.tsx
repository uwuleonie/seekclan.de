...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      {/* Banner */}
      <div className="h-40 w-full relative" style={{ background: user.banner_url ? `url(${user.banner_url}) center/cover` : 'linear-gradient(135deg, #4F46E5, #7C3AED, #C026D3)' }} />
      ...
      <div className="max-w-3xl mx-auto px-8">
        ...
        <div className="flex items-end justify-between -mt-12 mb-6">
          <div className="relative">
            <img
              src={`https://mc-heads.net/avatar/${user.minecraft_username || user.username}/80`}
              alt={user.username}
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>{user.username}</h1>
            {clanMember && (
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {/* Clan-Stufe */}
          {clanMember && (
            <div className="card rounded-2xl p-4 col-span-1">
              <p className="text-xs font-medium mb-2" style={{ color: 'var(--muted)' }}>Clan-Stufe</p>
              <div className="flex items-center gap-2 mb-2">
                <img src={`${SUPABASE_URL}/stufe${stufeIndex}.png`} alt="" className="w-6 h-6" />
              ...
              <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--muted-bg)' }}>
                <div className="h-full rounded-full" style={{ width: `${stufePercent}%`, background: 'linear-gradient(135deg,#4F46E5,#7C3AED)' }} />
              </div>
          ...
          <div className="card rounded-2xl p-4 col-span-1">
            ...
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--muted-bg)' }}>
              <div className="h-full rounded-full" style={{ width: `${levelProgress.percent}%`, background: 'linear-gradient(135deg,#C026D3,#7C3AED)' }} />
            </div>
          ...
          {clanMember && (
            <div className="card rounded-2xl p-4 col-span-1">
              <p className="text-xs font-medium mb-2" style={{ color: 'var(--muted)' }}>Im Clan seit</p>
              <p className="text-sm font-bold" style={{ color: 'var(--foreground)' }}>{daysSince(clanMember.join_date)} Tagen</p>
              <p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>{new Date(clanMember.join_date).toLocaleDateString('de-DE')}</p>
            </div>
          )}
        </div>
        ...
        {badges.length > 0 && (
          <div className="card rounded-2xl p-5 mb-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold" style={{ color: 'var(--foreground)' }}>Abzeichen</h2>
              <Link href={`/${username}/abzeichen`} className="text-xs hover:opacity-70" style={{ color: 'var(--muted)' }}>Alle anzeigen →</Link>
            </div>
              ...
              {badges.slice(0, 8).map(badge => (
                <div key={badge.id} title={badge.name}
                  ...
                  {badge.icon_url.startsWith('http') ? (
                    <img src={badge.icon_url} alt={badge.name} className="w-7 h-7 rounded-lg object-contain" />
                  ) : (
        ...
        {/* Spotify */}
        <SpotifyBlock username={username} />
        {/* Freunde */}
  ...
  function SpotifyBlock({ username }: { username: string }) {
