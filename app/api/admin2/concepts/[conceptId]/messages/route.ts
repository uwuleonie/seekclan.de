...
// GET /api/admin2/concepts/[conceptId]/messages
  ...
  try {
    ...
    const reactionsResult = await pool.query(
      `SELECT r.message_id, r.emoji, r.user_id
       FROM admin_concept_message_reactions r
       JOIN admin_concept_messages m ON m.id = r.message_id
       WHERE m.concept_id = $1`,
      [conceptId]
    )
    ...
    const messages = messagesResult.rows.map(m => {
      ...
      const attachmentUrl = m.attachment_filename ? getPublicUrl('chat-media', `concepts/${conceptId}/${m.attachment_filename}`) : null
    ...
    const pollsResult = await pool.query(
      `SELECT p.id, p.title, p.ends_at, p.created_at, p.created_by, u.username AS created_by_username
       ...
       WHERE p.concept_id = $1 AND p.type = 'poll'
...
// POST /api/admin2/concepts/[conceptId]/messages
...
export async function POST(
  ...
  try {
    if (file) {
      ...
      await saveFile('chat-media', `concepts/${conceptId}/${attachmentFilename}`, buffer)
    }

    const result = await pool.query(
      `INSERT INTO admin_concept_messages (concept_id, user_id, content, attachment_filename, attachment_name, attachment_size)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
      [conceptId, user.id, content, attachmentFilename, attachmentName, attachmentSize]
    )
