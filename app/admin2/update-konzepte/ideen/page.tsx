...
export default function UpdatesIdeenPage() {
  ...
  return (
    <div className="max-w-5xl">
      <Link href="/admin2/update-konzepte" className="text-sm inline-flex items-center gap-1 mb-6 hover:opacity-70 transition-all" style={{ color: 'var(--muted)' }}>
        ← Zurück zu Update-Konzepte
      </Link>

      <div className="flex items-start justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-1" style={{ color: 'var(--foreground)' }}>Unkonzipierte Ideen</h1>
          <p style={{ color: 'var(--muted)' }}>
            Ideen, die noch in kein Update-Konzept überführt wurden.
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {ideas.map(idea => {
            ...
            const href = idea.source === 'ticket' ? `/admin/support/${idea.id}` : `/admin2/update-konzepte/ideen/${idea.id}`
            return (
